...
# print(dim(a_debias)[1])
# print(dim(a_debias)[2])
# b <- xts(a[5,5,],times_ptype)
# print(b)
...
for (r in 1:dim(a_debias)[1]) {
  ...
  for (c in 1:dim(a_debias)[2]) {
    ...
    if (! all(is.nan(obs_rc))) {
      if (! all(is.nan(mod_rc))){
        if (! all(is.na(mod_rc))){
      print("I'm in the fucking loop for debiasing")
      ...
      # a_debias[r,c,]
    }}else{print("no fucking loop done")}
  }}
