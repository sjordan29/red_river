...
lm = LogonManager()
lm.logoff()
lm.logon_with_openid('https://esgf-data.dkrz.de/esgf-idp/openid/XXX', password='XXX')# , bootstrap=True)
...
proj_ls = ["CESM1(CAM5)",
           ...
           "MRI-AGCM3-2H", "MRI-AGCM3-2S", "MRI-CGCM3", "MRI-ESM1", "NorESM1-M", "NorESM1-ME"] # , "BNU-ESM", "BCC-CSM1.1(m)", "CCSM4", "CESM1(BGC)", 
...
#         files = os.listdir(os.path.join(esd.cfg.path_cmip6_archive, v))
#         proj_files = [os.path.join(esd.cfg.path_cmip6_archive, v, f) for f in files if projection_name in f]
